...
class PoincareBall(Manifold):
    def __init__(self,
                 ...
                 eps: float = 1e-5) -> None:
        if curvature == 0.0:
            raise TypeError("c=0 means we're using Euclidean Geometry.")
    ...
    def conformal_factor(self,
        ...
        return torch.clamp(
            2 / (1 - self.curvature * torch.sum(x * x, dim=dim, keepdim=keepdim)), min=self.eps
            )
    ...
    def hyperplane(self,
        ...
        denominator = (1 - self.curvature * sum_norm) * a_norm
    ...
    def __repr__(self) -> str:
        return "PoincareBallManifold, conformal_factor={}".format(self.curvature)
    
    def __eq__(self, other) -> bool:
        return self.curvature == other.conf_factor
